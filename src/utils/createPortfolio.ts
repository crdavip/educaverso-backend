...
export const createPortfolio = () => {
  return async (ctx: Koa.Context, next: () => Promise<void>) => {
    ...
    const isPublishRequest =
      ctx.request.url.startsWith("/content-manager/collection-types/api::portfolio.portfolio/actions/publish") &&
    ...
    try {
      ...
      const latestPortfolio = await strapi.entityService.findMany("api::portfolio.portfolio", {
        sort: { createdAt: "desc" },
        limit: 1,
      });
      ...
      await strapi.db.query("api::portfolio.portfolio").update({
      ...
      ctx.params = {
        model: "api::portfolio.portfolio",
        id: portfolio.documentId.toString(),
      };
